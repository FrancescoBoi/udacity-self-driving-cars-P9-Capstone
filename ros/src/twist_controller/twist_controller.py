...
class Controller(object):
    ...
    def control(self, current_vel, dbw_enabled, linear_vel, angular_vel):
        ...
        if linear_vel == 0. and current_vel<0.1:
            ...
            #we are going faster than we want (val_error<0) and the PID is letting up on
            # the throttle but we want to slow down.
            throttle = 0.
            decel = max(vel_error, self.decel_limit)
            brake = abs(decel)*self.vehicle_mass*self.wheel_radius #torque
            #abs cuz we want brake to be positive whilst decel can be negative
